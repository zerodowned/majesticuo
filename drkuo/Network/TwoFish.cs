...
namespace Encryption
{
    class TwoFish
    {
private Encryption.TwofishEncryption engine;
		...
		public void GameEncryption(uint seed) 
		{
			...
			byte[] iv = new byte[0];
			engine = new Encryption.TwofishEncryption(128, ref key, ref iv, CipherMode.ECB, Encryption.TwofishBase.EncryptionDirection.Encrypting);
        ...
        public void clientEncryptData(ref byte[] buffer, int length) 
		{
			for (int i = 0; i < length; ++i) 
			{
				// Recalculate table
